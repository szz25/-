...
function wallet_pay(_this) {
  console.log(_this.properties.price)
  wx.request({
    url: 'http://47.96.77.123:8080/html/user/sendBag',
    method:'POST',
    data:{
      openId: app.globalData.num,
      paymentKey: _this.data.wallets_password,
      totalMoney: _this.properties.price.totalMoney, 
      number: _this.properties.price.number,
      extraInfo: _this.properties.price.extraInfo
    },
    header:{
    ...
    success:function(res){
      if (res.data.data) {
        wx.navigateTo({
          url: _this.properties.url + "?bagNumber=" + res.data.data,
        })
      } else {
        wx.showToast({
